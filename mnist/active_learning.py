...
def random_sample_active_learning(model, X_train, Y_train, X_pool, Y_pool, X_test, Y_test, iters=10, k=50):
...
def maximum_entropy_active_learning(model, X_train, Y_train, X_pool, Y_pool, X_test, Y_test, iters=10, k=50):
    all_accuracy = np.array([])
    for i in range(iters):
        print("Active learning iteration %d" % i)
        print("Total data used so far: %d" % X_train.shape[0])
...
def maximum_meanvar_active_learning(model, X_train, Y_train, X_pool, Y_pool, X_test, Y_test, iters=10, k=50):
    all_accuracy = np.array([])
    for i in range(iters):
        print("Active learning iteration %d" % i)
        print("Total data used so far: %d" % X_train.shape[0])
...
def BALD_active_learning(model, X_train, Y_train, X_pool, Y_pool, X_test, Y_test, iters=10, k=50):
    all_accuracy = np.array([])
    for i in range(iters):
        print("Active learning iteration %d" % i)
        print("Total data used so far: %d" % X_train.shape[0])
...
def BALD_layer_active_learning(model, X_train, Y_train, X_pool, Y_pool, X_test, Y_test, iters=10, k=50):
    all_accuracy = np.array([])
    for i in range(iters):
        print("Active learning iteration %d" % i)
        print("Total data used so far: %d" % X_train.shape[0])
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        (X_train, Y_train), (X_pool, Y_pool), (_, _), (X_test, Y_test) = load_data(cnn=False)
        model.optimize(X_train, Y_train)
        BALD_layer_active_learning(model, X_train, Y_train, X_pool, Y_pool, X_test, Y_test)
